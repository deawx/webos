...
{ 
    ...
    $(config.element).attr('dialog', 'dialog')
    ...
    { 
        if (!isDown) return $(config.element).find('.move-block').hide();
        
		$(config.element).find('.move-block').show()
...
export function resize(config) 
{
    ...
    var mouseMoveEvent = function (event) 
    {
		...
		if ( !isDown )
		    return $(config.element).find('.move-block').hide();
